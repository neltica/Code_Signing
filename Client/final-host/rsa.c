...
long long rsa_modExp(long long b, long long e, long long m)
{
  ...
  //printf("b:%c:%lld e:%c:%lld m:%c:%lld\n",(char)b,b,(char)e,e,(char)m,m);
  if (b < 0 || e < 0 || m <= 0){
    /*
    if(b<0)
    {
                     ...
                     const struct private_key_class *priv)
{
  ...
  for(i=0; i < message_size; i++){
    encrypted[i] = rsa_modExp(message[i], priv->exponent, priv->modulus);
  }
  return encrypted;
}
                  ...
                  const struct public_key_class *pub)
{
  ...
  for(i=0; i < message_size/8; i++){
    temp[i] = rsa_modExp(message[i], pub->exponent, pub->modulus);
  }
...
int check_rsa_key(struct private_key_class *priv, struct public_key_class *pub,int sha1_length)
{
  long long *encrypted;
  char *decrypted;
  unsigned char *sha1_tmp_str=(unsigned char*)malloc(sizeof(unsigned char)*sha1_length*2);
  memset(sha1_tmp_str,'f',sizeof(char)*sha1_length*2);

  encrypted=rsa_encrypt(sha1_tmp_str,sizeof(char)*sha1_length*2,priv);
  if(!encrypted)
  {
    return 1;
  }
  decrypted=rsa_decrypt(encrypted,8*(sizeof(char)*sha1_length*2),pub);
  if(!decrypted)
  {
    return 1;
  }

  if(strcmp(sha1_tmp_str,decrypted)!=0)
  {
    return 1;
  }
  return 0;
